...
_COLOR_PALETTES = {
    "Astralis (Par défaut)": PALETTE,
    "Plasma": ["#0D0887", "#6A00A8", "#B12A90", "#E16462", "#FCA636", "#F0F921"],
    "Viridis": ["#440154", "#414487", "#2A788E", "#22A884", "#7AD151", "#FDE725"],
    "Inferno": ["#000004", "#1B0C41", "#4A0C6B", "#781C6D", "#BB3655", "#FC8961", "#FCFFA4"],
}


def _active_color_palette(main_win=None):
    """Retourne la palette de couleurs active selon les paramètres globaux."""
    if main_win is None:
        main_win = QApplication.activeWindow()
    name = "Astralis (Par défaut)"
    if main_win and hasattr(main_win, "local_settings"):
        name = main_win.local_settings.get("color_palette", name)
    return _COLOR_PALETTES.get(name, PALETTE)


def _body_color(index: int, main_win=None) -> str:
    palette = _active_color_palette(main_win)
...
def apply_dark_axes(ax, bg=FIG_PANEL, fg=TEXT_COL, grid_alpha=0.4):
    ...
    grid_visible = True
    if main_win and hasattr(main_win, "local_settings"):
        grid_visible = main_win.local_settings.get("grid_visible", True)
    ax.grid(grid_visible, alpha=grid_alpha, linestyle="--", linewidth=0.5, color=BORDER)
...
class OrbiteTab(QWidget):
    ...
    def _rebuild_body_checks(self):
        ...
        for i in range(self.data.n_bodies):
            ...
            color = _body_color(i, self.window())
            chk   = QCheckBox(name)
            chk.setChecked(True)
            chk.setStyleSheet(
    ...
    def _draw_2d(self, ax, d, idx_slice, idx_max, cx0, cy0, cz0, plane, trail_mode,
        ...
        lw_base = self.window().local_settings.get("line_width", 0.8) if hasattr(self.window(), "local_settings") else 0.8
        opacity_base = self.window().local_settings.get("trail_opacity", 0.8) if hasattr(self.window(), "local_settings") else 0.8
        for i in range(d.n_bodies):
            ...
            if trail_mode == 0:          # Ligne complète
                ...
                ax.plot(ha, hb, color=color, lw=lw_base * 0.8, alpha=opacity_base * 0.7)

            elif trail_mode == 1:        # Fenêtre glissante
                i0 = max(0, idx_max - window_pts)
                ax.plot(ha_full[i0:], hb_full[i0:], color=color, lw=lw_base, alpha=opacity_base)
                ...
                if n_seg > 1:
                    ...
                    for k in range(n_ch):
                        ...
                        alpha = min(1.0, (0.05 + 0.90 * (k / max(n_ch - 1, 1))) * opacity_base)
                        ax.plot(haw[s:e], hbw[s:e], color=base, lw=lw_base * 1.2, alpha=alpha)

            elif trail_mode == 3:        # Densité / nuage de points
                ha = ha_full[idx_slice]; hb = hb_full[idx_slice]
                ax.scatter(ha, hb, color=color, s=0.4, alpha=opacity_base * 0.15, linewidths=0)
        ...
        for i in range(d.n_bodies):
            ...
            if trail_mode == 0:
                ax.plot(xi_f[idx_slice], yi_f[idx_slice], zi_f[idx_slice],
                        color=color, lw=lw_base * 0.8, alpha=opacity_base * 0.56)
            elif trail_mode == 1:
                i0 = max(0, idx_max - window_pts)
                ax.plot(xi_f[i0:], yi_f[i0:], zi_f[i0:], color=color, lw=lw_base, alpha=opacity_base)
            elif trail_mode == 2:
                i0  = max(0, idx_max - window_pts)
                ...
                if n_seg > 1:
                    ...
                    for k in range(n_ch):
                        s = k * chunk; e = min(s + chunk + 1, n_seg + 1)
                        alpha = min(1.0, (0.05 + 0.90 * (k / max(n_ch - 1, 1))) * opacity_base)
                        ax.plot(xw[s:e], yw[s:e], zw[s:e], color=base, lw=lw_base * 1.2, alpha=alpha)
            elif trail_mode == 3:
                ax.scatter(xi_f[idx_slice], yi_f[idx_slice], zi_f[idx_slice],
                           color=color, s=0.4, alpha=opacity_base * 0.15, linewidths=0)

        # Proportions physiques 1:1:1 — évite l'étirement artificiel de l'axe Z par Matplotlib 3D
...
class DistanceTab(QWidget):
    ...
    def _draw_pair_on_ax(self, ax, t_ans, i, j, lbl, dist, unit, log_y, shared_y,
        ...
        color = _body_color(i + j, self.window())
...
TOOLTIPS = {
    ...
    "dt_max":        "Pas de temps maximum (s) — borne haute de l'intégrateur adaptatif Sundman.\n"
                     "Doit résoudre l'orbite du corps le plus rapide du système.\n"
                     "Planètes (UA) : ~7180 s (2 h). Satellite (Lune) : ~900 s (15 min).\n"
                     "Binaire serrée / Io : 120–1800 s selon les vitesses.",
    "dt_min":        "Pas de temps minimum (s) — précision maximale près des rapprochements.\n"
                     "Planètes : ~10 s. Satellites : 0,1 s. Binaires / marées extrêmes : 0,01 s.",
    "softening":     "Adoucissement gravitationnel en mètres.\nÉvite les divergences quand deux corps sont très proches.",
    "dist_seuil_m": "Distance (m) en dessous de laquelle dt est réduit automatiquement.\n"
                    "Planètes larges : ~0,005 UA (~750 000 km).\n"
                    "Terre–Lune : ~2,5×10⁸ m. Jupiter–Io : ~3×10⁸ m.\n"
                    "Binaires : 0,05–0,1 UA ou quelques×10⁹ m selon le scénario.",
    ...
    "incl_deg":      "Inclinaison ORBITALE du plan de l'orbite (degrés).\n"
                     "≠ obliquité de l'axe de rotation (réglée via spin_axis).\n"
                     "Terre : 0° (écliptique). Mars : ~1,85°. Lune : ~5,15° vs plan Terre–Soleil.\n"
...
class SimulationTab(QWidget):
    ...
    def _build_ui(self):
        ...
        self.cb_preset.addItems([
            "Soleil - Terre - Mars (Physique Réelle)",
            "Soleil - Terre - Lune (Marées & Co-rotation)",
            "Soleil - Terre - Mars - Jupiter (Système Interne)",
            "Étoile binaire (Déformation de marée)",
            "Système compact 3 corps (Chaos & Kozai-Lidov)",
            "Comète active (Dégazage & Perte de masse)",
            "Binaire à transfert de masse (Roche)",
            "Jupiter & Io (Aplatissement J2 & Marée extrême)",
        ])
        ...
        self.sp_dt_max = spin(1.0, 1e8, 7180.0, 1, "s", TOOLTIPS["dt_max"])
        self.sp_dt_min = spin(0.001, 1e6, 10.0, 3, "s", TOOLTIPS["dt_min"])
        ...
        hint(fl_int,
             "  dt_max doit résoudre l'orbite la plus rapide (satellite > planète > binaire).\n"
             "  dist_seuil_m réduit dt quand deux corps s'approchent sous ce seuil.")
        ...
        QTimer.singleShot(0, self._apply_local_global_defaults)
        self._update_runtime_info()

    def _apply_local_global_defaults(self):
        """Applique les préférences globales locales si settings.json ne les surcharge pas."""
        win = self.window()
        if not win or not hasattr(win, "local_settings"):
            return
        loc = win.local_settings
        sim_cfg = {}
        path = self._active_settings_path()
        if os.path.isfile(path):
            try:
                with open(path, "r", encoding="utf-8") as f:
                    sim_cfg = json.load(f).get("simulation", {})
            except (json.JSONDecodeError, OSError):
                pass
        if "output_dir" not in sim_cfg:
            self.le_output_dir.setText(loc.get("default_output_dir", "outputs"))
    ...
    def _apply_preset(self, name: str):
        """Applique un preset de scénario physique complet avec tous les paramètres avancés.

        Règles dt / dist_seuil_m (intégrateur Sundman adaptatif) :
        - Planètes (UA) : dt_max ~7180 s, dt_min ~10 s, seuil ~0,005 UA.
        - Satellites rapides (Lune) : dt_max ~900 s, dt_min ~0,1 s, seuil ~2,5×10⁸ m.
        - Jupiter–Io / binaires serrées : dt_max ~120 s, dt_min ~0,01 s.
        - incl_deg = inclinaison ORBITALE (≠ obliquité spin_axis).
        """
        presets = {
            "Soleil - Terre - Mars (Physique Réelle)": {
                "simulation": {
                    "duree_ans": 5.0, "dt_max": 7180.0, "dt_min": 10.0,
                    "softening": 1000.0, "alpha": 0.1, "dist_seuil_m": 0.005 * UA,
                    "enable_pn1": False
                },
                "bodies": [
                    {
                        "name": "Soleil", "mass": 1.989e30, "rayon": 6.957e8,
                        "pos": [0.0, 0.0, 0.0], "use_auto_vel": False, "vel_manual": [0.0, 0.0, 0.0],
                        "incl_deg": 0.0, "sens": 1, "j2r2": 0.0, "mdot": -2.0e9, "mdot_loss": True,
                        "spin_rate": 2.87e-6, "spin_axis": [0.0, 0.0, 1.0], "k2": 0.0, "inertia_factor": 0.4
                    },
                    {
                        "name": "Terre", "mass": 5.972e24, "rayon": 6.371e6,
                        "pos": [1.0 * UA, 0.0, 0.0], "use_auto_vel": True, "vel_manual": [0.0, 0.0, 0.0],
                        "incl_deg": 0.0, "sens": 1, "j2r2": 4.39e10, "mdot": 0.0, "mdot_loss": True,
                        "spin_rate": 7.2921e-5, "spin_axis": [0.3977, 0.0, 0.9175], "k2": 0.30, "inertia_factor": 0.3308
                    },
                    {
                        "name": "Mars", "mass": 6.418e23, "rayon": 3.389e6,
                        "pos": [1.524 * UA, 0.0, 0.0], "use_auto_vel": True, "vel_manual": [0.0, 0.0, 0.0],
                        "incl_deg": 1.85, "sens": 1, "j2r2": 2.45e9, "mdot": 0.0, "mdot_loss": True,
                        "spin_rate": 7.088e-5, "spin_axis": [0.4258, 0.0, 0.9048], "k2": 0.17, "inertia_factor": 0.366
                    }
                ]
            },
            "Soleil - Terre - Lune (Marées & Co-rotation)": {
                "simulation": {
                    "duree_ans": 2.0, "dt_max": 900.0, "dt_min": 0.1,
                    "softening": 500.0, "alpha": 0.05, "dist_seuil_m": 2.5e8, "enable_pn1": False
                },
                "bodies": [
                    {
                        "name": "Soleil", "mass": 1.989e30, "rayon": 6.957e8,
                        "pos": [0.0, 0.0, 0.0], "use_auto_vel": False, "vel_manual": [0.0, 0.0, 0.0],
                        "incl_deg": 0.0, "sens": 1, "j2r2": 0.0, "mdot": 0.0, "mdot_loss": True,
                        "spin_rate": 2.87e-6, "spin_axis": [0.0, 0.0, 1.0], "k2": 0.0, "inertia_factor": 0.4
                    },
                    {
                        "name": "Terre", "mass": 5.972e24, "rayon": 6.371e6,
                    ...
                    },
                    {
                        ...
                        "pos": [1.0 * UA + 3.844e8, 0.0, 0.0], "parent_index": 1, "pos_rel": [3.844e8, 0.0, 0.0],
                        "use_auto_vel": True, "vel_manual": [0.0, 0.0, 0.0],
                        "incl_deg": 5.145, "sens": 1, "j2r2": 0.0, "mdot": 0.0, "mdot_loss": True,
                        "spin_rate": 2.6617e-6, "spin_axis": [0.026, 0.0, 0.999], "k2": 0.03, "inertia_factor": 0.394
                    }
                ]
            },
            "Soleil - Terre - Mars - Jupiter (Système Interne)": {
                "simulation": {
                    "duree_ans": 12.0, "dt_max": 7180.0, "dt_min": 10.0,
                    "softening": 1000.0, "alpha": 0.1, "dist_seuil_m": 0.005 * UA,
                    "enable_pn1": False
                },
                "bodies": [
                    ...
                    {
                        "name": "Jupiter", "mass": 1.8982e27, "rayon": 7.1492e7,
                        "pos": [5.203 * UA, 0.0, 0.0], "use_auto_vel": True, "vel_manual": [0.0, 0.0, 0.0],
                        "incl_deg": 1.305, "sens": 1, "j2r2": 1.5e15, "mdot": 0.0, "mdot_loss": True,
                        "spin_rate": 1.7585e-4, "spin_axis": [0.054, 0.0, 0.998], "k2": 0.49, "inertia_factor": 0.264
                    }
                ]
            },
            "Étoile binaire (Déformation de marée)": {
                "simulation": {
                    "duree_ans": 1.0, "dt_max": 1800.0, "dt_min": 1.0,
                    "softening": 100000.0, "alpha": 0.05, "dist_seuil_m": 0.05 * UA,
                    "enable_pn1": False
                },
                "bodies": [
                    {
                        "name": "Étoile A", "mass": 1.989e30, "rayon": 7.0e8,
                        "pos": [0.4 * UA, 0.0, 0.0], "use_auto_vel": False, "vel_manual": [0.0, 15000.0, 0.0],
                        "incl_deg": 0.0, "sens": 1, "j2r2": 4.9e16, "mdot": 0.0, "mdot_loss": True,
                        "spin_rate": 3.0e-5, "spin_axis": [0.0, 0.0, 1.0], "k2": 0.75, "inertia_factor": 0.4
                    },
                    {
                        "name": "Étoile B", "mass": 1.5e30, "rayon": 6.0e8,
                        "pos": [-0.5 * UA, 0.0, 0.0], "use_auto_vel": False, "vel_manual": [0.0, -20000.0, 0.0],
                        "incl_deg": 0.0, "sens": 1, "j2r2": 3.6e16, "mdot": 0.0, "mdot_loss": True,
                        "spin_rate": 4.5e-5, "spin_axis": [0.0, 0.0, 1.0], "k2": 0.75, "inertia_factor": 0.4
                    }
                ]
            },
            "Système compact 3 corps (Chaos & Kozai-Lidov)": {
                "simulation": {
                    "duree_ans": 5.0, "dt_max": 900.0, "dt_min": 0.1,
                    "softening": 1000.0, "alpha": 0.05, "dist_seuil_m": 0.1 * UA,
                    "enable_pn1": False
                },
                "bodies": [
                    {
                        "name": "Étoile Centrale", "mass": 1.989e30, "rayon": 6.957e8,
                        "pos": [0.0, 0.0, 0.0], "use_auto_vel": False, "vel_manual": [0.0, 0.0, 0.0],
                        "incl_deg": 0.0, "sens": 1, "j2r2": 0.0, "mdot": 0.0, "mdot_loss": True,
                        "spin_rate": 2.87e-6, "spin_axis": [0.0, 0.0, 1.0], "k2": 0.0, "inertia_factor": 0.4
                    },
                    {
                        "name": "Géante Interne A", "mass": 1.898e28, "rayon": 8.0e7,
                        "pos": [0.8 * UA, 0.0, 0.0], "use_auto_vel": True, "vel_manual": [0.0, 0.0, 0.0],
                        "incl_deg": 12.0, "sens": 1, "j2r2": 1.8e15, "mdot": 0.0, "mdot_loss": True,
                        "spin_rate": 1.0e-4, "spin_axis": [0.0, 0.0, 1.0], "k2": 0.49, "inertia_factor": 0.26
                    },
                    {
                        "name": "Géante Externe B", "mass": 1.2e28, "rayon": 7.5e7,
                        "pos": [1.3 * UA, 0.0, 0.0], "use_auto_vel": True, "vel_manual": [0.0, 0.0, 0.0],
                        "incl_deg": 41.0, "sens": -1, "j2r2": 1.4e15, "mdot": 0.0, "mdot_loss": True,
                        "spin_rate": 8.0e-5, "spin_axis": [0.0, 0.0, -1.0], "k2": 0.49, "inertia_factor": 0.26
                    }
                ]
            },
            "Comète active (Dégazage & Perte de masse)": {
                "simulation": {
                    "duree_ans": 12.0, "dt_max": 14400.0, "dt_min": 1.0,
                    "softening": 50000.0, "alpha": 0.02, "enable_pn1": False
                },
                "bodies": [
                    {
                        "name": "Soleil", "mass": 1.989e30, "rayon": 6.957e8,
                    ...
                    {
                        "name": "Comète active", "mass": 5.0e14, "rayon": 8000.0,
                        "pos": [0.5 * UA, 0.0, 0.0], "use_auto_vel": False, "vel_manual": [0.0, 48000.0, 0.0],
                        "incl_deg": 18.5, "sens": 1, "j2r2": 0.0, "mdot": -5.0e6, "mdot_loss": True,
                        "spin_rate": 3.0e-4, "spin_axis": [0.577, 0.577, 0.577], "k2": 0.0, "inertia_factor": 0.4
                    }
                ]
            },
            "Binaire à transfert de masse (Roche)": {
                "simulation": {
                    "duree_ans": 0.1, "dt_max": 120.0, "dt_min": 0.01,
                    "softening": 100000.0, "alpha": 0.05, "dist_seuil_m": 5.0e9,
                    "enable_pn1": True
                },
                "bodies": [
                    {
                        "name": "Étoile A (Donneuse)", "mass": 3.978e30, "rayon": 1.2e9,
                        "pos": [2.5e9, 0.0, 0.0], "use_auto_vel": False, "vel_manual": [0.0, 160000.0, 0.0],
                        "incl_deg": 0.0, "sens": 1, "j2r2": 4.9e16, "mdot": -2.0e18, "mdot_loss": True,
                        "spin_rate": 1.5e-4, "spin_axis": [0.0, 0.0, 1.0], "k2": 0.50, "inertia_factor": 0.28
                    },
                    {
                        "name": "Étoile B (Accrétrice)", "mass": 2.7846e30, "rayon": 1.5e7,
                        "pos": [-3.57e9, 0.0, 0.0], "use_auto_vel": False, "vel_manual": [0.0, -228500.0, 0.0],
                        "incl_deg": 0.0, "sens": 1, "j2r2": 0.0, "mdot": 2.0e18, "mdot_loss": False,
                        "spin_rate": 8.0e-3, "spin_axis": [0.0, 0.0, 1.0], "k2": 0.05, "inertia_factor": 0.35
                    }
                ]
            },
            "Jupiter & Io (Aplatissement J2 & Marée extrême)": {
                "simulation": {
                    "duree_ans": 0.05, "dt_max": 120.0, "dt_min": 0.01,
                    "softening": 1000.0, "alpha": 0.05, "dist_seuil_m": 3.0e8,
                    "enable_pn1": False
                },
                "bodies": [
                    {
                        "name": "Jupiter", "mass": 1.8982e27, "rayon": 7.1492e7,
                        "pos": [0.0, 0.0, 0.0], "use_auto_vel": False, "vel_manual": [0.0, 0.0, 0.0],
                        "incl_deg": 0.0, "sens": 1, "j2r2": 1.5e15, "mdot": 0.0, "mdot_loss": True,
                        "spin_rate": 1.7585e-4, "spin_axis": [0.0, 0.0, 1.0], "k2": 0.49, "inertia_factor": 0.264
                    },
                    {
                        "name": "Io", "mass": 8.9319e22, "rayon": 1.821e6,
                        "pos": [4.217e8, 0.0, 0.0], "parent_index": 0, "pos_rel": [4.217e8, 0.0, 0.0],
                        "use_auto_vel": True, "vel_manual": [0.0, 0.0, 0.0],
                        "incl_deg": 0.05, "sens": 1, "j2r2": 0.0, "mdot": 0.0, "mdot_loss": True,
                        "spin_rate": 4.11e-5, "spin_axis": [0.0, 0.0, 1.0], "k2": 0.15, "inertia_factor": 0.378
                    }
                ]
            }
        }
        ...
        self.console.append(f"Preset appliqué avec succès : {name}")
    ...
    def _stop(self):
        win = self.window()
        if (win and hasattr(win, "local_settings")
                and win.local_settings.get("confirm_stop_simulation", False)):
            reply = QMessageBox.question(
                self, "Arrêter la simulation",
                "Voulez-vous vraiment arrêter la simulation en cours ?",
    ...
    def _on_finished(self, ok: bool, msg: str):
        ...
        # Signal audio à la fin
        beep_active = True
        if hasattr(self.window(), "local_settings"):
            beep_active = self.window().local_settings.get("simulation_beep", True)
        if beep_active and ok:
            try:
                import winsound
...
""")
        ...
        scroll_layout.setContentsMargins(8, 10, 8, 8)
        scroll_layout.setSpacing(18)

        # ── GroupBox 1 : Affichage ──
        grp_display = QGroupBox("Affichage & Graphiques")
        lay_display = QFormLayout(grp_display)
        lay_display.setContentsMargins(12, 6, 12, 12)
        lay_display.setSpacing(10)

        self.cb_lang = QComboBox()
        self.cb_lang.addItems(["Français", "English (Coming soon)"])
        self.cb_lang.setCurrentText(current_settings.get("lang", "Français"))
        lay_display.addRow("Langue de l'interface :", self.cb_lang)

        self.sp_line_width = QDoubleSpinBox()
        self.sp_line_width.setRange(0.2, 4.0)
        self.sp_line_width.setSingleStep(0.2)
        self.sp_line_width.setDecimals(1)
        self.sp_line_width.setValue(current_settings.get("line_width", 0.8))
        self.sp_line_width.setSuffix(" px")
        lay_display.addRow("Épaisseur des trajectoires :", self.sp_line_width)

        self.sp_trail_opacity = QDoubleSpinBox()
        self.sp_trail_opacity.setRange(0.1, 1.0)
        self.sp_trail_opacity.setSingleStep(0.1)
        self.sp_trail_opacity.setDecimals(1)
        self.sp_trail_opacity.setValue(current_settings.get("trail_opacity", 0.8))
        self.sp_trail_opacity.setSuffix(" (alpha)")
        lay_display.addRow("Opacité des tracés :", self.sp_trail_opacity)

        self.chk_grid = QCheckBox("Afficher la grille de repère")
        self.chk_grid.setChecked(current_settings.get("grid_visible", True))
        lay_display.addRow("", self.chk_grid)

        self.cb_palette = QComboBox()
        self.cb_palette.addItems(["Astralis (Par défaut)", "Plasma", "Viridis", "Inferno"])
        self.cb_palette.setCurrentText(current_settings.get("color_palette", "Astralis (Par défaut)"))
        lay_display.addRow("Palette de couleurs :", self.cb_palette)

        self.cb_unit_default = QComboBox()
        self.cb_unit_default.addItems(["UA", "km", "m"])
        self.cb_unit_default.setCurrentText(current_settings.get("unit_default", "UA"))
        lay_display.addRow("Unité par défaut :", self.cb_unit_default)

        scroll_layout.addWidget(grp_display)

        # ── GroupBox 2 : Confort & Navigation ──
        grp_comfort = QGroupBox("Confort & Navigation")
        lay_comfort = QFormLayout(grp_comfort)
        lay_comfort.setContentsMargins(12, 6, 12, 12)
        lay_comfort.setSpacing(10)

        self.chk_remember_last_file = QCheckBox("Rouvrir le dernier fichier .parquet au démarrage")
        self.chk_remember_last_file.setChecked(current_settings.get("remember_last_file", False))
        lay_comfort.addRow("", self.chk_remember_last_file)

        self.cb_startup_tab = QComboBox()
        self.cb_startup_tab.addItems([
            "Orbites", "Simulation", "Distances", "Energie", "Dernier onglet utilisé",
        ])
        self.cb_startup_tab.setCurrentText(current_settings.get("default_startup_tab", "Orbites"))
        lay_comfort.addRow("Onglet au démarrage :", self.cb_startup_tab)

        self.chk_switch_orbites_on_load = QCheckBox("Basculer sur « Orbites » après ouverture d'un fichier")
        self.chk_switch_orbites_on_load.setChecked(
            current_settings.get("switch_to_orbites_on_load", True))
        lay_comfort.addRow("", self.chk_switch_orbites_on_load)

        self.chk_remember_window = QCheckBox("Mémoriser la taille et la position de la fenêtre")
        self.chk_remember_window.setChecked(current_settings.get("remember_window_size", True))
        lay_comfort.addRow("", self.chk_remember_window)

        self.cb_font_scale = QComboBox()
        self.cb_font_scale.addItems(["Compact", "Normal", "Confort"])
        self.cb_font_scale.setCurrentText(current_settings.get("ui_font_scale", "Normal"))
        lay_comfort.addRow("Taille du texte :", self.cb_font_scale)

        self.chk_confirm_stop = QCheckBox("Demander confirmation avant d'arrêter une simulation")
        self.chk_confirm_stop.setChecked(current_settings.get("confirm_stop_simulation", False))
        lay_comfort.addRow("", self.chk_confirm_stop)

        scroll_layout.addWidget(grp_comfort)

        # ── GroupBox 3 : Système ──
        grp_system = QGroupBox("Système & Performances")
        lay_system = QFormLayout(grp_system)
        lay_system.setContentsMargins(12, 6, 12, 12)
        lay_system.setSpacing(10)

        self.le_default_output_dir = QLineEdit()
        self.le_default_output_dir.setText(current_settings.get("default_output_dir", "outputs"))
        lay_system.addRow("Dossier de sortie :", self.le_default_output_dir)

        self.chk_beep = QCheckBox("Bip sonore à la fin de la simulation")
        self.chk_beep.setChecked(current_settings.get("simulation_beep", True))
        lay_system.addRow("", self.chk_beep)
        ...
        # Zone des boutons (Sauvegarder / Annuler)
    ...
    def get_settings(self) -> dict:
        return {
            ...
            "line_width": round(self.sp_line_width.value(), 1),
            "trail_opacity": round(self.sp_trail_opacity.value(), 1),
            "grid_visible": self.chk_grid.isChecked(),
            "color_palette": self.cb_palette.currentText(),
            "unit_default": self.cb_unit_default.currentText(),
            "remember_last_file": self.chk_remember_last_file.isChecked(),
            "default_startup_tab": self.cb_startup_tab.currentText(),
            "switch_to_orbites_on_load": self.chk_switch_orbites_on_load.isChecked(),
            "remember_window_size": self.chk_remember_window.isChecked(),
            "ui_font_scale": self.cb_font_scale.currentText(),
            "confirm_stop_simulation": self.chk_confirm_stop.isChecked(),
            "default_output_dir": self.le_default_output_dir.text().strip() or "outputs",
            "simulation_beep": self.chk_beep.isChecked(),
        }


class MainWindow(QMainWindow):

    def __init__(self, initial_file: str = "", splash_report=None):
        super().__init__()
        ...
        def _splash(pct: int, stage: str):
            if splash_report:
                splash_report(pct, stage)
                QApplication.processEvents()

        _splash(30, "Interface principale…")
        ...
        self.btn_settings.clicked.connect(self._ouvrir_parametres)
        ...
        # ── Onglets ──────────────────────────────────────
        _splash(35, "Préparation des onglets…")
        self.tabs = QTabWidget()
        tab_specs = [
            ("tab_orbite",    OrbiteTab,     "Orbites",    "Orbites"),
            ("tab_distance",  DistanceTab,   "Distances",  "Distances"),
            ("tab_energie",   EnergieTab,    "Energie",    "Énergie"),
            ("tab_vitesse",   VitesseTab,    "Vitesses",   "Vitesses"),
            ("tab_stabilite", StabiliteTab,  "Stabilite",  "Stabilité"),
            ("tab_phase",     PhaseTab,      "Phase",      "Phase"),
            ("tab_densite",   DensiteTab,    "Densite",    "Densité"),
            ("tab_resume",    ResumeTab,     "Resume",     "Résumé"),
            ("tab_sim",       SimulationTab, "Simulation", "Simulation"),
        ]
        for i, (attr, cls, tab_name, splash_name) in enumerate(tab_specs):
            setattr(self, attr, cls())
            self.tabs.addTab(getattr(self, attr), tab_name)
            pct = 35 + int(50 * (i + 1) / len(tab_specs))
            _splash(pct, f"Onglet {splash_name}…")
        ...
        _splash(88, "Configuration initiale…")

        self._apply_ui_font_scale()
        self._apply_ui_language()
        self._apply_startup_tab()
        if self.lbl_file.text() == "Aucun fichier chargé":
            self.tab_orbite.cb_unit.setCurrentText(
                self.local_settings.get("unit_default", "UA"))

        # Chargement initial
        if initial_file and os.path.exists(initial_file):
            self._load_file(initial_file)
        elif self.local_settings.get("remember_last_file"):
            last = self.local_settings.get("last_parquet_path", "")
            if last and os.path.isfile(last):
                self._load_file(last)

    def _settings_file_path(self) -> str:
        """Retourne le chemin d'accès au fichier de préférences utilisateur."""
        return os.path.join(APP_DIR, "app_settings_local.json")

    def _load_local_settings(self):
        """Charge les paramètres locaux enregistrés, ou applique les valeurs par défaut."""
        default_settings = {
            "lang": "Français",
            "line_width": 0.8,
            "trail_opacity": 0.8,
            "grid_visible": True,
            "color_palette": "Astralis (Par défaut)",
            "unit_default": "UA",
            "remember_last_file": False,
            "last_parquet_path": "",
            "default_startup_tab": "Orbites",
            "last_active_tab": "Orbites",
            "switch_to_orbites_on_load": True,
            "remember_window_size": True,
            "window_geometry": "",
            "ui_font_scale": "Normal",
            "confirm_stop_simulation": False,
        ...
        if os.path.isfile(path):
            try:
                with open(path, "r", encoding="utf-8") as f:
                    loaded = json.load(f)
                self.local_settings = {**default_settings, **loaded}
            except Exception:
    ...
    def _save_local_settings(self):
        """Enregistre les préférences utilisateur dans le fichier local JSON."""
        try:
            with open(self._settings_file_path(), "w", encoding="utf-8") as f:
                json.dump(self.local_settings, f, indent=2, ensure_ascii=False)
        except Exception as e:
            print(f"Erreur d'écriture des paramètres locaux : {e}")

    def _ouvrir_parametres(self):
        dialog = SettingsDialog(self.local_settings, self)
        if dialog.exec_() == QDialog.Accepted:
            self.local_settings = dialog.get_settings()
            self._save_local_settings()
            saved_msg = (
                "⚙️ Settings saved successfully."
                if self._ui_lang_is_english()
                else "⚙️ Paramètres enregistrés avec succès."
            )
            self.statusBar().showMessage(saved_msg, 3000)

            self.tab_orbite.cb_unit.setCurrentText(self.local_settings.get("unit_default", "UA"))

            # Propager dossier de sortie vers l'onglet Simulation
            if hasattr(self.tab_sim, "_apply_local_global_defaults"):
                self.tab_sim._apply_local_global_defaults()

            self._apply_ui_font_scale()
            self._apply_ui_language()
            self._apply_startup_tab()

            # Re-dessiner les graphiques pour appliquer grille, opacité et épaisseur
            for tab_name in (
                "tab_orbite", "tab_distance", "tab_energie", "tab_vitesse",
                "tab_stabilite", "tab_phase", "tab_densite", "tab_resume",
            ):
    ...
    def _on_tab_changed(self, index: int):
        if index >= 0:
            self.local_settings["last_active_tab"] = self.tabs.tabText(index)

    def _ui_lang_is_english(self) -> bool:
        return str(self.local_settings.get("lang", "Français")).startswith("English")

    def _apply_ui_language(self):
        """Applique la langue sur les éléments principaux de la barre d'outils."""
        en = self._ui_lang_is_english()
        empty_labels = ("Aucun fichier chargé", "No file loaded")
        if en:
            self.setWindowTitle("Astralis — N-body Orbital Simulation Dashboard")
            self.btn_open.setText("📂  Open a file…")
            self.btn_settings.setText("⚙️  Settings")
            if self.lbl_file.text() in empty_labels:
                self.lbl_file.setText("No file loaded")
        else:
            self.setWindowTitle("Astralis — Simulation Orbitale N-corps")
            self.btn_open.setText("📂  Ouvrir un fichier…")
            self.btn_settings.setText("⚙️  Paramètres")
            if self.lbl_file.text() in empty_labels:
                self.lbl_file.setText("Aucun fichier chargé")
        if not self.lbl_info.text():
            ready_msg = (
                "Astralis · Ready — Open a .parquet file to get started."
                if en else
                "Astralis · Prêt — Ouvrez un fichier .parquet pour commencer."
            )
            self.statusBar().showMessage(ready_msg)

    def _default_open_dir(self) -> str:
        """Dossier proposé à l'ouverture de fichier (dernier chemin ou dossier de sortie)."""
        last = self.local_settings.get("last_parquet_path", "")
        if last and os.path.isdir(os.path.dirname(last)):
            return os.path.dirname(last)
        out = self.local_settings.get("default_output_dir", "outputs")
        candidate = out if os.path.isabs(out) else os.path.join(PROJECT_ROOT, out)
        if os.path.isdir(candidate):
            return candidate
        return _outputs_dir()

    def _apply_startup_tab(self):
        choice = self.local_settings.get("default_startup_tab", "Orbites")
        if choice == "Dernier onglet utilisé":
            choice = self.local_settings.get("last_active_tab", "Orbites")
        for i in range(self.tabs.count()):
            if self.tabs.tabText(i) == choice:
                self.tabs.setCurrentIndex(i)
                return

    def _apply_ui_font_scale(self):
        sizes = {"Compact": 11, "Normal": 13, "Confort": 15}
        pt = sizes.get(self.local_settings.get("ui_font_scale", "Normal"), 13)
        font = QApplication.font()
        font.setPointSize(pt)
        QApplication.setFont(font)

    def closeEvent(self, event):
        if self.local_settings.get("remember_window_size", True):
            geom = self.saveGeometry().toBase64().data().decode("ascii")
            self.local_settings["window_geometry"] = geom
        idx = self.tabs.currentIndex()
        if idx >= 0:
            self.local_settings["last_active_tab"] = self.tabs.tabText(idx)
        self._save_local_settings()
        super().closeEvent(event)

    def _show_main_window(self):
        """Affiche la fenêtre en respectant la géométrie mémorisée."""
        if (self.local_settings.get("remember_window_size")
                and self.local_settings.get("window_geometry")):
            self.restoreGeometry(QByteArray.fromBase64(
                self.local_settings["window_geometry"].encode("ascii")))
            self.show()
        else:
            self.showMaximized()
    ...
    def _open_file(self):
        path, _ = QFileDialog.getOpenFileName(
            self, "Ouvrir une simulation", self._default_open_dir(),
    ...
    def _load_file(self, path: str):
        ...
        self.local_settings["last_parquet_path"] = os.path.abspath(path)
        self._save_local_settings()
        ...
        if self.local_settings.get("switch_to_orbites_on_load", True):
            self.tabs.setCurrentWidget(self.tab_orbite)
...
SPLASH_PROGRESS_FILE = os.path.join(APP_DIR, ".splash_progress.json")
SPLASH_READY_FLAG = os.path.join(APP_DIR, ".ready_flag")


def _write_splash_progress(progress: int, stage: str = ""):
    """Écrit la progression lue par splash.py pendant le démarrage."""
    progress = max(0, min(100, int(progress)))
    try:
        with open(SPLASH_PROGRESS_FILE, "w", encoding="utf-8") as f:
...
def main():
    ...
    splash_mode = args.splash
    if splash_mode:
        for path in (SPLASH_READY_FLAG, SPLASH_PROGRESS_FILE):
            try:
                if os.path.isfile(path):
                    os.remove(path)
            except OSError:
                pass
        _write_splash_progress(5, "Démarrage du moteur d'interface…")
    ...
    if splash_mode:
        _write_splash_progress(12, "Application du thème…")
    ...
    if splash_mode:
        _write_splash_progress(18, "Chargement des ressources…")
    ...
    splash_cb = _write_splash_progress if splash_mode else None
    if splash_mode:
        _write_splash_progress(22, "Construction du tableau de bord…")

    # ── Construction de la fenetre principale ────────────
    win = MainWindow(initial_file=args.file, splash_report=splash_cb)
    win.setWindowIcon(app_icon)

    if splash_mode:
        _write_splash_progress(92, "Finalisation de l'interface…")

        def _signal_ready():
            _write_splash_progress(100, "Prêt")
            try:
                with open(SPLASH_READY_FLAG, "w", encoding="utf-8") as f:
                    f.write("ready")
            except OSError:
                pass
