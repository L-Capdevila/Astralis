...
class SplashWindow(QWidget):
    ...
    def __init__(self):
        ...
        self._phase2 = 0.0
        self._phase3 = 0.0
        self._anim_done   = False   # animation terminée
    ...
    def _check_ready(self):
        if os.path.isfile(READY_FLAG):
            ...
            # Si l'animation est déjà finie → fermeture immédiate
            # Sinon on attend qu'elle finisse (_tick s'en occupe)
            if self._anim_done:
                self._close()
    ...
    def _tick(self):
        ...
        elif self._phase3 < 100:
            self._phase3 = min(100.0, self._phase3 + self._SPEED3)
    ...
    def mousePressEvent(self, _event):
        if not self._anim_done:
            self._phase = self._phase2 = self._phase3 = 100.0
            self._anim_done = True
            if self._dash_ready:
    ...
    def paintEvent(self, _event):
        ...
        # ── Barre ────────────────────────────────────────
        # Si anim finie mais dashboard pas encore prêt : barre pulse
        if self._anim_done and not self._dash_ready:
            pulse = 0.6 + 0.4 * math.sin(self._tick_count * 0.08)
            progress = pulse
            bar_label = "En attente du dashboard..."
        else:
            progress = self._phase3 / 100.0
        ...
        # Label barre
        if self._phase3 > 5:
            la2 = int(min(1.0, self._phase3 / 30) * 120)
            p.setFont(font_load)
            p.setPen(QColor(0, 229, 255, la2))
            p.drawText(0, y_label, W, label_h + 4, Qt.AlignHCenter, bar_label)
